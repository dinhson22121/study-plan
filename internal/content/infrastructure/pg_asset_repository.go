...
func (r *PgAssetRepository) Create(ctx context.Context, a *domain.UploadedAsset) error {
	const q = `
		INSERT INTO uploaded_asset
			(id, object_key, bucket_name, original_filename, content_type, file_size,
			 checksum_sha256, status, uploaded_by, entity_type, entity_id, storage_provider, created_at)
	...
	_, err := r.db.Exec(ctx, q, a.ID, a.ObjectKey, a.BucketName, a.OriginalFilename, a.ContentType,
		a.FileSize, a.ChecksumSHA256, string(a.Status), a.UploadedBy, string(a.EntityType), entityID, a.StorageProvider, a.CreatedAt)
	if err != nil {
...
func (r *PgAssetRepository) GetByID(ctx context.Context, id string) (*domain.UploadedAsset, error) {
	const q = `
		SELECT id, object_key, bucket_name, original_filename, content_type, file_size,
		       checksum_sha256, status, uploaded_by, entity_type, entity_id::text, storage_provider, created_at, verified_at, deleted_at
...
func (r *PgAssetRepository) LinkEntity(ctx context.Context, id string, entityType domain.AssetEntityType, entityID string) error {
	const q = `UPDATE uploaded_asset SET entity_type = $2, entity_id = $3 WHERE id = $1`
	tag, err := r.db.Exec(ctx, q, id, string(entityType), entityID)
	if err != nil {
		return shared.ErrInternal.WithCause(err)
	}
	if tag.RowsAffected() == 0 {
...
func scanAsset(row rowScanner) (*domain.UploadedAsset, error) {
	...
	err := row.Scan(&a.ID, &a.ObjectKey, &a.BucketName, &a.OriginalFilename, &a.ContentType, &a.FileSize,
		&a.ChecksumSHA256, &status, &a.UploadedBy, &entityType, &entityID, &a.StorageProvider, &a.CreatedAt, &a.VerifiedAt, &a.DeletedAt)
	if err != nil {
	...
	a.EntityType = domain.AssetEntityType(entityType)
	if entityID != nil {
		a.EntityID = *entityID
	}
	return &a, nil
